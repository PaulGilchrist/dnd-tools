...
function ChargeSystem({ magicItem }) {
    if (!magicItem.chargeSystem) return null;
    
    const charges = magicItem.chargeSystem;
    const parts = [];
...
function SpellCasting({ magicItem }) {
    if (!magicItem.spellCasting) return null;
    
    const { save_dc, attack_bonus, spells } = magicItem.spellCasting;
    const parts = [];
...
function SavingThrows({ magicItem }) {
    if (!magicItem.savingThrows || magicItem.savingThrows.length === 0) return null;
    
    const parts = ['<b>Saving Throws:</b>'];
    magicItem.savingThrows.forEach(st => {
...
function AdvantageDisadvantage({ magicItem }) {
    if (!magicItem.advantageDisadvantage) return null;
    
    const { advantage_on, disadvantage_on } = magicItem.advantageDisadvantage;
    const parts = [];
...
function ItemSlot({ magicItem }) {
    if (!magicItem.itemSlot) return null;
    
    return <div><b>Item Slot:</b> {magicItem.itemSlot}</div>;
}
...
function UsageLimit({ magicItem }) {
    if (!magicItem.usageLimit) return null;
    
    return <div><b>Usage Limit:</b> {magicItem.usageLimit}</div>;
}
...
function ActionTypes({ magicItem }) {
    if (!magicItem.actionTypes || magicItem.actionTypes.length === 0) return null;
    
    return <div><b>Action Types:</b> {magicItem.actionTypes.join(', ')}</div>;
}
...
function AttunementRequirements({ magicItem }) {
    if (!magicItem.attunementRequirements) return null;
    
    return <div><b>Attunement Requirement:</b> {magicItem.attunementRequirements}</div>;
}
